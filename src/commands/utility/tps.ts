...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('tps')
		.setDescription('Displays ticks per second')
			...
			option.setName('server')
				...
				return interaction.reply('You do not have the required role to use this command.');
					...
					switch(server){
						...
						const data = await runRconCommand(serverType, port, password,  (switchTracking == 'yes' ) ? "seclosia enable tps" : "seclosia get tps");
